...
import (
	...
	"github.com/modeneis/uminer/src/stratum"
)
...
func (sc *StratumClient) Start() {
	...
	sc.stratumclient.ErrorCallback = func(err error) {
		log.WithError(err).Error("Error in connection to stratumserver:")
	...
	log.Println("INFO: Connecting to", sc.connectionstring)
	err := sc.stratumclient.Dial(sc.connectionstring)
	if err != nil {
		log.WithError(err).WithField("connectionstring", sc.connectionstring).Error("Failed to connect")
	}
	...
	if err != nil {
		log.WithError(err).Error("Got Error in response from stratum:")
		sc.stratumclient.Close()
		return
	}
	reply, ok := result.([]interface{})
	if !ok || len(reply) < 3 {
		log.WithError(err).Error("Invalid response from stratum:", result)
		sc.stratumclient.Close()
		return
	}
	...
	if sc.extranonce1, err = stratum.HexStringToBytes(reply[1]); err != nil {
		log.WithError(err).Error("Invalid extrannonce1 from startum")
		sc.stratumclient.Close()
		return
	}
	...
	if !ok {
		log.WithError(err).Error("Invalid extranonce2_size from stratum", reply[2], "type", reflect.TypeOf(reply[2]))
		sc.stratumclient.Close()
		return
	}
	...
	go func() {
		...
		if err != nil {
			log.WithError(err).Error("Unable to authorize:", err)
			sc.stratumclient.Close()
			return
		}
		log.Info("Authorization of", sc.User, ":", result)
	}()
...
func (sc *StratumClient) subscribeToStratumDifficultyChanges() {
	sc.stratumclient.SetNotificationHandler("mining.set_difficulty", func(params []interface{}) {
		if params == nil || len(params) < 1 {
			log.WithField("params", params).Error("No difficulty parameter supplied by stratum server")
			return
		}
		diff, ok := params[0].(float64)
		if !ok {
			log.WithField("params", params).Error("Invalid difficulty supplied by stratum server")
			return
		}
		log.WithField("diff", diff).Info("Stratum server changed difficulty")
...
func (sc *StratumClient) subscribeToStratumJobNotifications() {
	sc.stratumclient.SetNotificationHandler("mining.notify", func(params []interface{}) {
		log.Info("New job received from stratum server")
		if params == nil || len(params) < 9 {
			log.WithField("params", params).Error("Wrong number of parameters supplied by stratum server")
			return
		}
		...
		if sj.JobID, ok = params[0].(string); !ok {
			log.WithField("params", params).Error("Wrong job_id parameter supplied by stratum server")
			return
		}
		if sj.PrevHash, err = stratum.HexStringToBytes(params[1]); err != nil {
			log.WithField("params", params).Error("Wrong prevhash parameter supplied by stratum server")
			return
		}
		if sj.Coinbase1, err = stratum.HexStringToBytes(params[2]); err != nil {
			log.WithField("params", params).Error("Wrong coinb1 parameter supplied by stratum server")
			return
		}
		if sj.Coinbase2, err = stratum.HexStringToBytes(params[3]); err != nil {
			log.WithField("params", params).Error("Wrong coinb2 parameter supplied by stratum server")
			return
		}
		...
		if !ok {
			log.WithField("params", params).Error("Wrong merkle_branch parameter supplied by stratum server")
			return
		}
		//sj.MerkleBranch = make([][]byte, len(merklebranch), len(merklebranch))
		sj.MerkleBranch = make([][]byte, len(merklebranch))
		...
		if sj.Version, ok = params[5].(string); !ok {
			log.WithField("params", params).Error("Wrong version parameter supplied by stratum server")
			return
		}
		if sj.NBits, ok = params[6].(string); !ok {
			log.WithField("params", params).Error("Wrong nbits parameter supplied by stratum server")
			return
		}
		if sj.NTime, err = stratum.HexStringToBytes(params[7]); err != nil {
			log.WithField("params", params).Error("Wrong ntime parameter supplied by stratum server")
			return
		}
		if sj.CleanJobs, ok = params[8].(bool); !ok {
			log.WithField("params", params).Error("Wrong clean_jobs parameter supplied by stratum server")
			return
		}
...
func difficultyToTarget(difficulty float64) (target Target, err error) {
	...
	if err != nil {
		log.WithError(err).WithField("target", target).Error("Could not set difficultyToTarget")
	}
	return target, err
}
...
func (sc *StratumClient) setDifficulty(difficulty float64) {
	...
	if err != nil {
		log.WithError(err).WithField("difficulty", difficulty).Errorf("Error setting difficulty")
	}
	sc.mutex.Lock()
	defer sc.mutex.Unlock()
	sc.target = target
...
func (sc *StratumClient) SubmitHeader(header []byte, job interface{}) (err error) {
	...
	//TODO: develop better way to handle this donations
